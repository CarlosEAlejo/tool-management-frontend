...
export const ToolTable = ({ tools, onView, onEdit, onDelete }: ToolTableProps) => (
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-[var(--border-subtle)]">
          ...
          <tbody className="divide-y divide-[var(--border-subtle)] bg-transparent">
            {tools.length > 0 ? (
              tools.map((tool) => (
                <ToolRow key={tool.id} tool={tool} onView={() => onView(tool)} onEdit={() => onEdit(tool)} onDelete={() => onDelete(tool)} />
              ))
            ) : (
              <tr>
